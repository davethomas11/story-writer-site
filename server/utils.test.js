...
describe('sanitizeFolderName', () => {
    ...
    test('defaults to "story-id" if title is empty or non-alphanumeric', () => {
        expect(sanitizeFolderName('', '789')).toBe('story-789');
        expect(sanitizeFolderName('!!!', 'xyz')).toBe('story-xyz');
    });

    test('handles null or undefined title', () => {
        expect(sanitizeFolderName(null, '999')).toBe('story-999');
        expect(sanitizeFolderName(undefined, '888')).toBe('story-888');
    });
});
